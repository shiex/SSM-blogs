...
<html>
				...
				<div class="user-data">
					...
					<div class="user-item">
						<div class="item-lable">
							<label>原创</label><br /><label>${user.original_count}</label>
						</div>
						<div class="item-lable">
							<label>关注</label><br /><label>${user.follow_count}</label>
						</div>
						<div class="item-lable">
							<label>粉丝</label><br /><label>${user.fans_count}</label>
						</div>
						<div class="item-lable">
							<label>访问量</label><br /><label>${user.visit_count}</label>
						</div>
			...
			<div class="article-con">
   					...
   					<#if articles??>
						<#list articles as article>
							<li class="clerfix">
								...
								<div class="userData">
									<div class="day">
										<label>发布时间：${article.issue_time?datetime}</label>
									</div>
									<div class="read-count">
										<label>阅读量</label><br>
										<label>${article.read_count}</label>
									</div>
								</div>
							</li>
						</#list>
				...
				<div class="notice">
					...
					<ul class="notice-con">
						<#if notices??>
							<#list notices as n>
								<li class="notice-li" onclick="M.readNotice(${n.id})">${n.title}</li>
							</#list>
							<#else >
								<li class="notice-li">暂无公告</li>
						</#if>
					</ul>
				</div>
			</div>
	...
	<div class="dig">
		<div class="notice">
			...
			<div class="notice-button">
				<button onclick="M.call()">关闭</button>
			</div>
		</div>
	</div>
	</body>
	...
	<script>
		var M = {};
		M.readNotice = function (noticeId) {
			Af.rest("notice/read.do", noticeId, function (data) {
				$('.dig .notice-title').text(data.title);
				$('.dig .notice-content').text(data.content);
				$('.dig').show();
			});
		}
		M.call = function () {
