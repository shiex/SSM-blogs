    ...
    <div class="left">
        <div class="userItem">
            <div class="user-photo">
                ...
                <div class="u-name"><a href="userPage/${article.user_id}.html">${article.user.username}</a></div>
                <#if isFollow == 1>
            ...
            <div class="user-text">
                <div class="u-text">
                    ...
                    <label> ${article.user.original_count} </label>
                </div>
                <div class="u-text">
                    <label class="u-lable">关注</label><br>
                    <label> ${article.user.follow_count} </label>
                </div>
                <div class="u-text">
                    <label class="u-lable">粉丝</label><br>
                    <label> ${article.user.fans_count} </label>
                </div>
            </div>
        </div>
                ...
                <#if articles??>
                    <#list articles as art>
                        <div class="arcl">
                            ...
                            <label style="margin-left: 10px; color: #aba5a5;">阅读量：${art.read_count}</label>
                        </div>
                    </#list>
                </#if>
            </div>
...
<div class="content">
    <div class="article-top">
        ...
        <div class="userData">
            <div class="issue_time">发布时间：${article.issue_time?datetime}</div>
            <div class="username2">
                <a href="userPage/${article.user_id}.html"> ${article.user.username} </a>
            </div>
            <div class="original">${article.original?then('原创','转载')}</div>
            <div class="read_count">阅读量：${article.read_count}</div>
            <#if (isCollect > 0)>
    ...
    <div class="comment-reply">
        <#if list_comment??>
        <div class="ct">
            <#list list_comment as comment>
            <div class="item-comt">
                <div class="comt-t" style="padding: 0px;">
                    <div class="comt-img">
                        <img src='${comment.user.head_portrait}' onclick="M.loadUserPage(${comment.user_id})"/>
                    </div>
                    <div class="comt-con">
                        <div>
                            <div class="show">
                                <div class="user" by_userId="${comment.user_id}" by_username="${comment.user.username}">
                                    <label class="lable-name" onclick="M.loadUserPage(${comment.user_id})">${comment.user.username}</label>
                                ...
                                <#if (comment.reply_count > 0)>
                                    <label class="isFindR" onclick="M.findReply(this,${comment.id})" isFindR="0">
                                        查看回复<${comment.reply_count}> </label>
                                </#if>
                            </div>
                        </div>
                        <div>
                            <label style="color: #7db797;"> ${comment.comment_time?datetime} </label>
                        </div>
                    </div>
                    <div class="comt-x">
                        <!-- 回复内容区 -->
                ...
                <#if (comment.status > 0)>
                    ...
                    <div class="appld"> ${comment.appluad_count} </div>
                <#else>
...
<script>
    ...
    M.addReply = function(e, comment_id) {
        ...
        req.by_user_id = $(reply_hf).attr('by_userId');
        req.by_username = $(reply_hf).attr('by_username');
        Af.rest("reply.add", req, function (data) {
            window.location.reload(true);
        });
    }
    ...
    M.findReply = function(e, comtId) {
        ...
        if(isFindR == 0) {
            ...
            var req = {};
            req.id = comtId;
            Af.rest("reply.find", req, function (data) {
      ...
      M.showReply = function(cx, data) {
        var tmp = new AfTemplate(
              "<div class='comt-text'>"
            +	"<div class='text-img'>"
            +		"<img src='{#head_portrait}' style='cursor: pointer' onclick='M.loadUserPage({#user_id})'/>"
            +	"</div>"
            +   "<div class='reply'>"
            +		"<label class='reply-name' user_id={#user_id} onclick='M.loadUserPage({#user_id})'> {#username} </label>"
            +		"<label> 回复 {#by_username}：</label>"
            +		"<label> {#content} </label>"
